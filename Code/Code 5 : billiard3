import numpy as np
import matplotlib.pyplot as plt


class BILLIARD(object):
    def __init__(self):
        self.dt, self.time=0.001, 50
        self.n=int(self.time/self.dt)
        self.x, self.y, self.vx, self.vy = [np.sqrt(2)], [0], [0], [1]
    def calculate(self):
        for i in range(self.n):
            self.nextx = self.x[-1]+self.vx[-1]*self.dt
            self.nexty = self.y[-1]+self.vy[-1]*self.dt
            self.nextvx, self.nextvy = self.vx[-1], self.vy[-1]
            if self.nextx**2/3+self.nexty**2>1:
                self.nextx=self.x[-1]
                self.nexty=self.y[-1]
                while not(self.nextx**2/3+self.nexty**2>1):
                    self.nextx=self.nextx+self.nextvx*self.dt/100
                    self.nexty=self.nexty+self.nextvy*self.dt/100
                self.norm=np.array([self.nextx,3*self.nexty])
                self.norm=self.norm/np.sqrt(np.dot(self.norm,self.norm))
                self.v=np.array([self.nextvx,self.nextvy])
                self.v_perpendicular=np.dot(self.v,self.norm)*self.norm
                self.v_parrallel=self.v-self.v_perpendicular
                self.v_perpendicular=-self.v_perpendicular
                [self.nextvx,self.nextvy]=self.v_parrallel+self.v_perpendicular
            self.x.append(self.nextx)
            self.y.append(self.nexty)
            self.vx.append(self.nextvx)
            self.vy.append(self.nextvy)
    def plot_position(self,_ax):
        _ax.plot(self.x,self.y,'-b',label='Ellipse'+r'$a=2,b=1$')
        _ax.plot(np.sqrt(3)*np.cos(np.linspace(0,2*np.pi,200)),np.sin(np.linspace(0,2*np.pi,200)),'-k',lw=5)
    def plot_phase(self,_ax):
        self.phase_x, self.phase_vx = [], []
        for i in range(len(self.x)):
            if abs(self.y[i]-0.)<1E-3 and abs(self.vx[i])<0.95:
                self.phase_x.append(self.x[i])
                self.phase_vx.append(self.vx[i])
        _ax.plot(self.phase_x,self.phase_vx,'ob',markersize=2,label='Ellipse'+r'$a=2,b=1$')

fig= plt.figure(figsize=(15,6))
ax1=plt.axes([0.1,0.15,0.35,0.7])
ax2=plt.axes([0.6,0.15,0.35,0.7])

comp=BILLIARD()
comp.calculate()
comp.plot_position(ax1)
comp.plot_phase(ax2)


ax1.set_xlim(-2.0,2.0)
ax1.set_ylim(-1.1,1.1)
ax2.set_xlim(-2.0,2.0)
ax2.set_ylim(-1.1,1.1) 
ax1.set_xlabel(r'$x (m)$',fontsize=14)
ax1.set_ylabel(r'$y (m)$',fontsize=14)
ax1.set_title('Elliptical stadium - trajectory: '+r'$a^2 =3,b^2 = 1$',fontsize=14)
ax2.set_xlabel(r'$x (m)$',fontsize=14)
ax2.set_ylabel(r'$v_x (m/s)$',fontsize=14)
ax2.set_title('Elliptical stadium - phase space plot: '+r'$a^2 =3,b^2 = 1$',fontsize=14)

plt.show()

